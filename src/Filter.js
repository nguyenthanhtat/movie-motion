...
const Filter = ({setActiveGenre,activeGenre,setFiltered,filtered,popular}) => {
    useEffect(() => {
        if(activeGenre===0){
            setFiltered(popular);
            return;
    ...
    <div className="filter-container">
        <button
        className={activeGenre ===0 ?"action":""}
        onClick={()=> setActiveGenre(0)}>All</button>    
        <button 
        className={activeGenre ===35 ?"action":""}
        onClick={()=> setActiveGenre(35)}>Comedy</button>    
        <button
        className={activeGenre ===28 ?"action":""}
